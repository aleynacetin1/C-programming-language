...
WARNING!! Please use for megabyte 'MB', for kilobyte 'KB', for byte 'B' .
MB=1000000 byte
KB=1000 byte
*/
...
#include <stdio.h>
 ...
 int KB=1000;
   printf(" -DATA TYPES- \n");
   printf("'i' for int\n'c' for char\n'd' for double\n's' for short\n");
   printf("Type of data and Number of variables: ");
   scanf("%c %d",&character ,&number);
   
   if(character=='i'){
       size=number*sizeof(int);
   }
   else if(character=='c'){
       size=number*sizeof(char);
   }
   else if(character=='d'){
       size=number*sizeof(double);
   }
   else if(character=='s'){
       size=number*sizeof(short);
   }
   
  
  if (size >= MB) {
    printf("%d MB and %d KB and %d B\n", size/MB, (size/KB)%KB, size%KB);
  } else if (size >= KB) {
    printf("%d KB and %d B\n", size/KB, size%KB);
  } else {
    printf("%d B\n", size);
