...
#include <stdio.h>
double meterstofeet(double,char);
double gramtopound(double,char);
double celciustofahrenheit(double,char);

int main(void){
    int number,i;
    double x;
    char unit;
    printf("How many number have: ");
    scanf("%d",&number);
    
    for(i=0;i<number;i++){
        scanf("%lf %c",&x ,&unit); //x=the number to be converted ;  unit=unit of the number
        if(sign=='m'){
           meterstofeet(x,unit);
        }else if(sign=='g'){
           gramtopound(x,unit);
        }else if(sign=='c'){
           celciustofahrenheit(x,unit);
        }
    }
    return 0;
}
double meterstofeet(double value,char character){
    double result;
    result=value*3.2808;
    printf("%.6lf ft\n",result);
    return result;
...
double gramtopound(double value,char character){
    double result;
...
double celciustofahrenheit(double value,char character){
    double result;
    result=32+1.8*value;
    printf("%.6lf f\n",result);
    return result;
