...
int main(void){
    ...
    int height,numberLeaflets;
    //enter heights of tree and the number of leaflets
    scanf("%d%d",&height,&numberLeaflets);
    
    if(height<=5&&numberLeaflets>=8)
      printf("Tinuviel");
    else if(height>=10&&numberLeaflets>=10)
        printf("Calaelen");
    else if(height<=8&&numberLeaflets<=5)
        printf("Falarion");
    else if(height>=12&&numberLeaflets<=7)
        printf("Dorthonion");
    else
        printf("Uncertain");
